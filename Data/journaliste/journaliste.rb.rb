...
contacts.each_with_index{|e,i| puts "Element: #{e}, Index: #{i}"}
puts array.index { |x| ["@epenser"].include?(x) }      


puts contacts.join.length

sorted = contacts.sort
puts sorted
puts contacts.length

puts contacts.count { |s| s.include?('_') }
puts contacts.count { |s| s.match?('[A-Z]') }

puts contacts.min 
contacts.min { |a, b| a.length <=> b.length }


puts contacts.find { |l| l.size == 3 }
puts contacts.find { |l| l.size == 6 }

puts array.sort_by{ |word| word.length }
p contacts.min_by(&:length)
p contacts.find { |l| l.size == 5}


#contacts.each do |contact|
 # puts count = contacts.count(contact)
  #name_frequency_hash[contact] = count
#end